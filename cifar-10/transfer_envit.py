...
model_ckpt = torch.load(ENVIT_PATH)
model_encoder.load_state_dict(model_ckpt['model_state_dict'])
...
def train(model, device, train_loader, optimizer, criterion, is_decoder=True):
    ...
    for batch_idx, (data, target) in pbar:
        ...
        if is_decoder:
            ...
            output = model.forward_masked(data)
...
if len(sys.argv) == 2:
    ...
    print(f"Loaded EViT checkpoint from {evit_ckpt_path}")
...
for epoch in range(start_epoch, 1+NUM_EPOCHS):
    ...
    test_loss_encoder, test_accuracy_encoder = test(model_encoder, device, val_loader, criterion, is_decoder=False)
    scheduler_encoder.step()
    lr_epoch = optimizer_encoder.param_groups[0]['lr']
    print(f"(EViT)  EPOCH {epoch:03d}/{NUM_EPOCHS:03d}, LR {lr_epoch:.4e} | T LOSS: {train_loss_encoder:.4f}, T ACC: {train_accuracy_encoder:.2f}%, V LOSS: {test_loss_encoder:.4f}, V ACC: {test_accuracy_encoder:.2f}%")
    print()

    # save model checkpoint
    evit_ckpt_path = f"./saves/transenvit/epoch_{epoch:03d}.pt"
    evit_ckpt = {
        'epoch': epoch,
