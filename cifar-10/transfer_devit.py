...
DEVIT_PATH = "./saves/devit/devit_best_85.70.pt"
ENVIT_PATH = "./saves/envit/evit_best_86.60.pt"
...
print("Loaded weights from DeViT and EnViT models")


# 모델 파라미터 수 계산 및 출력
num_params_decoder = sum(p.numel() for p in model_devit.parameters() if p.requires_grad)
print(f"Number of trainable parameters (Decoder): {num_params_decoder:,d}")

optimizer = optim.AdamW(model_devit.parameters(), lr=1e-4, weight_decay=1e-5)
...
if len(sys.argv) == 2:
    ...
    model_devit.load_state_dict(devit_ckpt['model_state_dict'])
    optimizer.load_state_dict(devit_ckpt['optimizer_state_dict'])
    scheduler.load_state_dict(devit_ckpt['scheduler_state_dict'])
...
for epoch in range(start_epoch, 1+NUM_EPOCHS):
    ...
    train_loss_decoder, train_accuracy_decoder = train(model_devit, device, train_loader, optimizer, criterion, is_decoder=True)
    test_loss_decoder, test_accuracy_decoder = test(model_devit, device, val_loader, criterion, is_decoder=True)
    scheduler.step()
    ...
    devit_ckpt_path = f"./saves/transdevit/epoch_{epoch:03d}.pt"
    devit_ckpt = {
        'epoch': epoch,
        'model_state_dict': model_devit.state_dict(),
